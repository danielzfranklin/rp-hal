...
fn UART0_IRQ() {
    ...
    // Since we read until the buffer is empty, the interrupt is cleared for us. The read empties the buffer, and hence clears the IRQ.
    //
    // With something like PWM we'd need to clear the IRQ ourselves with `cortex_m::peripheral::nvic::NVIC::unpend`.
}
